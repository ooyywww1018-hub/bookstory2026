...
import { 
  ...
  Users, Calendar, FileSpreadsheet, RefreshCw, Eye, Sparkles, CheckCircle2, Shield
} from 'lucide-react';
...
export const TeacherDashboard: React.FC<TeacherDashboardProps> = ({
  ...
  return (
    <div className="space-y-8 pb-12 animate-fade-in">
      ...
      <div className="p-6 sm:p-8 rounded-3xl bg-gradient-to-r from-[#f7f1e5] via-[#f2e7d5] to-[#ebdcc7] border border-[#d8c3a5] shadow-sm relative overflow-hidden">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="space-y-1">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-violet-100 text-violet-800 border border-violet-300 text-xs font-bold">
              <Shield className="w-3.5 h-3.5 text-violet-700" />
              <span>교사 전용 대시보드</span>
            </div>
            <h2 className="text-2xl sm:text-3xl font-extrabold text-[#4a3319] tracking-tight font-serif">
              학급 독서 현황 종합 분석 대시보드
            </h2>
            <p className="text-[#7a6043] text-xs sm:text-sm">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              className="px-3.5 py-2.5 bg-white hover:bg-[#f5ede1] text-[#593b1d] rounded-xl text-xs font-semibold border border-[#d9ccbd] transition-colors flex items-center gap-1.5"
            >
            ...
            <button
              ...
              className="px-4 py-2.5 bg-emerald-700 hover:bg-emerald-600 text-white rounded-xl text-xs font-bold shadow-md transition-all flex items-center gap-1.5"
            >
        ...
        <div className="mt-6 pt-6 border-t border-[#d8c3a5]/60 flex flex-wrap items-center gap-4">
          <div className="flex items-center gap-2">
            <span className="text-xs font-bold text-[#593b1d]">학년 필터:</span>
            <select
              value={selectedGrade}
              onChange={(e) => setSelectedGrade(e.target.value)}
              className="bg-white border border-[#d9ccbd] text-[#3b2713] text-xs rounded-xl px-3 py-2 focus:outline-none focus:border-[#8c6239]"
            >
          ...
          <button
            ...
            className="ml-auto text-xs text-[#8c7355] hover:text-rose-600 underline"
          >
            기본 샘플 데이터로 리셋
          </button>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="p-5 bg-white border border-[#e6dcce] rounded-2xl shadow-sm flex items-center justify-between">
          <div>
            <div className="text-xs text-[#8c7355] font-bold">총 누적 독서 권수</div>
            <div className="text-2xl font-black text-[#3b2713] mt-1">
              {stats.totalBooks}<span className="text-xs font-normal text-[#8c7355] ml-1">권</span>
            </div>
          </div>
          <div className="w-12 h-12 rounded-2xl bg-[#f5ede1] border border-[#d9ccbd] text-[#734e2b] flex items-center justify-center">
        ...
        <div className="p-5 bg-white border border-[#e6dcce] rounded-2xl shadow-sm flex items-center justify-between">
          <div>
            ...
            <div className="text-2xl font-black text-violet-700 mt-1 flex items-center gap-1">
              <span>{stats.averageRating}</span>
              <Star className="w-5 h-5 text-amber-500 fill-amber-400" />
            </div>
          </div>
          <div className="w-12 h-12 rounded-2xl bg-violet-50 border border-violet-200 text-violet-700 flex items-center justify-center">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="p-6 bg-white border border-[#e6dcce] rounded-3xl shadow-sm space-y-4">
              ...
              {stats.topCategories.map((item, idx) => (
                <div key={item.category} className="space-y-1">
                  <div className="flex justify-between text-xs text-[#593b1d]">
                    ...
                    <span>{item.count}권 ({item.percentage}%)</span>
                  </div>
            ...
            {stats.monthlyTrend.map((m) => {
              ...
              return (
                <div key={m.month} className="flex-1 flex flex-col items-center gap-2 h-full justify-end group">
                  ...
                  <span className="text-xs font-semibold text-[#8c7355]">{m.month}</span>
                </div>
              );
            })}
          </div>
      ...
      <div className="p-6 bg-white border border-[#e6dcce] rounded-3xl shadow-sm space-y-4">
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            <div className="relative flex-1 sm:w-64">
              <input
                ...
                className="w-full bg-[#fbf8f3] border border-[#d9ccbd] rounded-xl pl-9 pr-3 py-2 text-xs text-[#3b2713] placeholder-[#aa9580] focus:outline-none focus:border-[#8c6239]"
              />
        ...
        <div className="overflow-x-auto custom-scrollbar border border-[#e6dcce] rounded-2xl">
          <table className="w-full text-left text-xs text-[#4a3319]">
            <thead className="bg-[#f8f3eb] text-[#8c7355] font-bold border-b border-[#e6dcce]">
              <tr>
            ...
            <tbody className="divide-y divide-[#f0e6da]">
              {filteredRecords.length > 0 ? (
                filteredRecords.map((r) => (
                  <tr key={r.id} className="hover:bg-[#fbf8f3] transition-colors">
                    <td className="p-3 font-semibold text-[#3b2713] whitespace-nowrap">
                      {r.grade} {r.classNum} <strong className="text-[#734e2b]">{r.studentName}</strong>
                    </td>
                    ...
                    <td className="p-3 whitespace-nowrap">
                      <span className="px-2 py-0.5 rounded bg-[#f4ebe1] text-[#734e2b] border border-[#d9ccbd] text-[11px]">
                        {r.category}
                      </span>
                    </td>
                    <td className="p-3 whitespace-nowrap">
                    ...
                    <td className="p-3 text-[#6e5843] max-w-xs truncate">
                    ...
                    <td className="p-3 text-right whitespace-nowrap">
                      <div className="flex items-center justify-end gap-1">
                        <button
                          ...
                          className="p-1.5 bg-[#f5ede1] hover:bg-[#eadecc] text-[#734e2b] rounded-lg transition-colors"
                          title="상세보기"
                        >
